...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^devilian_") ; then
        DEVILIAN_BUILD=$(echo -n $1 | sed -e 's/^devilian_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $DEVILIAN_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        DEVILIAN_BUILD=
    fi
    export DEVILIAN_BUILD
